...
function decryptEntry(cipher, entry, startKey) {
  ...
  for (const prf of ["sha1", "sha256"]) {
    try {
      ...
      const sum = createHash("sha256").update(plain.subarray(0, Math.min(CHECKSUM_PREFIX_BYTES, plain.length))).digest();
...
async function parseImage(buffer, options) {
  try {
    const { parseImageDocument } = await import("./image-ocr-B7TMIG3F.js");
...
async function parsePdf(buffer, options) {
  ...
  try {
    const mod = await import("./parser-HT7GZOF2.js");
...
//# sourceMappingURL=chunk-W5NZYPRQ.js.map