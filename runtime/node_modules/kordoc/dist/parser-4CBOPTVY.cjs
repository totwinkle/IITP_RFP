...
var _chunkR4LXZRYPcjs = require('./chunk-R4LXZRYP.cjs');
...
require('./chunk-EV53YE3W.cjs');



var _chunkE6DONLTMcjs = require('./chunk-E6DONLTM.cjs');
...
async function extractPageImages(page, fnArray, argsArray, pageNumber, state, warnings) {
  ...
  for (let i = 0; i < fnArray.length; i++) {
    ...
    if (!rgba) continue;
    const png = _chunkR4LXZRYPcjs.encodePng.call(void 0, w, h, rgba);
    state.imageIndex++;
...
function cleanPdfText(text) {
  return mergeKoreanLines(
    ...
    return _chunkKE6DLZAWcjs.collapseEvenSpacing.call(void 0, line);
...
function applyLinkAnnotations(items, annots) {
  ...
  for (const a of annots) {
    ...
    const url = _chunkE6DONLTMcjs.sanitizeHref.call(void 0, a.url);
    if (!url) continue;
...
async function applyFormulaOcr(buffer, blocks, pageFilter, effectivePageCount, warnings, _onProgress) {
  const formulaMod = await Promise.resolve().then(() => _interopRequireWildcard(require("./formula-PDMZMBIP.cjs")));
  ...
  try {
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        timer = setTimeout(() => {
          loadingTask.destroy();
          reject(new (0, _chunkE6DONLTMcjs.KordocError)("PDF \uB85C\uB529 \uD0C0\uC784\uC544\uC6C3 (30\uCD08 \uCD08\uACFC)"));
        }, PDF_LOAD_TIMEOUT_MS);
      })
    ]);
  } finally {
...
async function parsePdfDocument(buffer, options) {
  ...
  try {
    ...
    if (pageCount === 0) throw new (0, _chunkE6DONLTMcjs.KordocError)("PDF\uC5D0 \uD398\uC774\uC9C0\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4.");
    ...
    for (let i = 1; i <= effectivePageCount; i++) {
      ...
      try {
        ...
        const items = _chunkKE6DLZAWcjs.normalizeItems.call(void 0, rawItems);
        const { visible, hiddenCount } = _chunkKE6DLZAWcjs.filterHiddenText.call(void 0, items, viewport.width, viewport.height);
        if (hiddenCount > 0) {
        ...
        if (pageArea > 0) {
          const imageRegions = _chunkKE6DLZAWcjs.extractImageRegions.call(void 0, opList.fnArray, opList.argsArray);
          let uncovered = 0;
        ...
        const pageBlocks = _chunkKE6DLZAWcjs.extractPageBlocksWithLines.call(void 0, visible, i, opList, viewport.width, viewport.height, void 0, _optionalChain([options, 'optionalAccess', _10 => _10.tables]) !== false);
        ...
        if (totalTextBytes > MAX_TOTAL_TEXT) throw new (0, _chunkE6DONLTMcjs.KordocError)("\uD14D\uC2A4\uD2B8 \uCD94\uCD9C \uD06C\uAE30 \uCD08\uACFC");
        parsedPages++;
        _optionalChain([options, 'optionalAccess', _11 => _11.onProgress, 'optionalCall', _12 => _12(parsedPages, totalTarget)]);
      } catch (pageErr) {
        if (pageErr instanceof _chunkE6DONLTMcjs.KordocError) throw pageErr;
    ...
    if (_optionalChain([options, 'optionalAccess', _13 => _13.ocr]) && ocrBuffer) {
      ...
      if (targets.size > 0) {
        try {
          const { runPdfOcr } = await Promise.resolve().then(() => _interopRequireWildcard(require("./pdf-ocr-6ZNGDVC6.cjs")));
    ...
    if (_optionalChain([options, 'optionalAccess', _14 => _14.removeHeaderFooter]) !== false && parsedPageCount >= 3) {
      const removed = _chunkKE6DLZAWcjs.removeHeaderFooterBlocks.call(void 0, blocks, pageHeights, warnings);
      for (let ri = removed.length - 1; ri >= 0; ri--) {
        blocks.splice(removed[ri], 1);
      }
    }
    _chunkKE6DLZAWcjs.mergeCrossPageTables.call(void 0, blocks);
    ...
    const medianFontSize = _chunkKE6DLZAWcjs.computeMedianFontSizeFromFreq.call(void 0, fontSizeFreq);
    if (medianFontSize > 0) {
      _chunkKE6DLZAWcjs.detectHeadings.call(void 0, blocks, medianFontSize);
    }
    _chunkKE6DLZAWcjs.detectMarkerHeadings.call(void 0, blocks);
    _chunkKE6DLZAWcjs.detectTableCaptions.call(void 0, blocks);
    _chunkKE6DLZAWcjs.detectKoreanListBlocks.call(void 0, blocks);
    const outline = blocks.filter((b) => b.type === "heading" && b.level && b.text).map((b) => ({ level: b.level, text: b.text, pageNumber: b.pageNumber }));
    sanitizeBlockControlChars(blocks);
    let markdown = cleanPdfText(_chunkR4LXZRYPcjs.blocksToMarkdown.call(void 0, blocks));
    return {
      markdown,
      blocks,
      metadata,
...
exports.cleanPdfText = cleanPdfText; exports.detectKoreanListBlocks = _chunkKE6DLZAWcjs.detectKoreanListBlocks; exports.detectTableCaptions = _chunkKE6DLZAWcjs.detectTableCaptions; exports.extractPdfMetadataOnly = extractPdfMetadataOnly; exports.mergeCrossPageTables = _chunkKE6DLZAWcjs.mergeCrossPageTables; exports.parsePdfDocument = parsePdfDocument; exports.removeHeaderFooterBlocks = _chunkKE6DLZAWcjs.removeHeaderFooterBlocks;
//# sourceMappingURL=parser-4CBOPTVY.cjs.map